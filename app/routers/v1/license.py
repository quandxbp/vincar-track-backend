...
async def list_licenses(page: int = 1, limit: int = 10):
    ...
    for l in activated_licenses:
        if (existing_user := await main.app.state.mongo_collections[config.USERS_COLLECTION].find_one(
            ...
            device_ids = existing_user.get('devices', [])
            if len(device_ids):
                devices = await main.app.state.mongo_collections[config.DEVICES_COLLECTION].find({"uuid": {"$in": device_ids}}).to_list(1000)
                existing_user['devices_data'] = devices
            l['user'] = existing_user
    return licenses


@router.post("/", response_description="Auto generate license")
async def create_license(data: LicenseCreate = Body(...)):
    created_license = await main.app.state.mongo_collections[config.LICENSE_COLLECTION].insert_one(jsonable_encoder(data))
    if (existing_license := await main.app.state.mongo_collections[config.LICENSE_COLLECTION].find_one(
...
async def get_license(code: str):
    if (l := await main.app.state.mongo_collections[config.LICENSE_COLLECTION].find_one({"code": code})) is not None:
        return l
...
async def get_license_by_username(username: str):
    if (license_data := await main.app.state.mongo_collections[config.LICENSE_COLLECTION].find_one({"username": username})) is not None:
        return license_data
...
async def update_license(code: str, data: LicenseUpdate = Body(...)):
    if (existing_license := await main.app.state.mongo_collections[config.LICENSE_COLLECTION].find_one({"code": code})) is not None:
        if existing_license.get("username"):
            ...
            update_result = await main.app.state.mongo_collections[config.LICENSE_COLLECTION].update_one({"code": code},
                                                                                                  {"$set": license_data})
